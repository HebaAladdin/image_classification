...
class AugmentedImageSequence(Sequence):
    ...
    def __init__(self, dataset_csv_file, label_columns, class_names, flipped_class_names, source_image_dir,
                 batch_size=16,
                 target_size=(224, 224), augmenter=None, flip_augmenter=None, verbose=0, steps=None,
        ...
        self.flip_augmenter = flip_augmenter
        self.flipped_class_names = flipped_class_names
    ...
    def __getitem__(self, idx):
        ...
        if self.flip_augmenter is not None:
            for image_index in range(batch_y.shape[0]):
                flip_prob = np.random.sample()
                if flip_prob > 0.5:
                    batch_y[image_index] = self.class_names.index(self.flipped_class_names[int(batch_y[image_index])])
                    batch_x[image_index] = self.flip_augmenter.augment_image(batch_x[image_index])  
    ...
    def convert_labels_to_numbers(self, y):
        sparse_labels = np.zeros(y.shape[0])
        for i in range(y.shape[0]):
            class_index=self.class_names.index(y[i][0])
            sparse_labels[i] = class_index
            self.class_counts[class_index] += 1
        return sparse_labels

    def prepare_dataset(self):
        df = self.dataset_df.sample(frac=1., random_state=self.random_state)
        self.x_path, self.y = df["image_path"].as_matrix(), self.convert_labels_to_numbers(df[self.label_columns].as_matrix())
